function [mu, sigma, rou, AEPE,Energy] = gqmap_gpuV2(options,I1,I2,GRDT)
...
sigmav = rand(M,N,'gpuArray') + (maxv-minv);
...
while 1
    ...
    sigmau(M_,N_) = min(max(sigmau(M_,N_) + dsigmau(M_,N_) * step *0.7,0.01),23);
    sigmav(M_,N_) = min(max(sigmav(M_,N_) + dsigmav(M_,N_) * step *0.7,0.01),23);
...
%         imwrite(flc,[options.dir,'/',num2str(it),'.png']);
    ...
    if reset==1, reset_para();end
end
toc;
    function reset_para()
        sigmau = sigmau.*0+(maxu-minu)/2;% make sure it's a large initialization
        sigmav = sigmau.*0+(maxv-minv)/2;
        pn = pn.*0;
        rou = rou.*0;
        it=1;
        fprintf('Sigma, pN and rou have been reset\n');
        reset = 0;
    end
...
%                             end
%                         end
%                         fval = -lambdad*WIWJ(k)*super/wdo;
            fval= -lambdad*WIWJ(k)*sqrt(epsn + (I1(m,n) - I2_cont(min(max(round((m+x2-1)*rfc2+1),1),MM),min(max(round((n+x1-1)*rfc2+1),1),NN)))^2);
    ...
    function [du1,du2,do1,do2,dp,eener] = edge_grad_spectral(u1,u2,o1,o2,p)
        ...
        for k=1:K2
            zi = s*XI(k)+t*XJ(k); zj = t*XI(k)+s*XJ(k);
            fval = -lambdas*WIWJ(k)*sqrt(epsn + (sqrt2*o1*zi+u1 - sqrt2*o2*zj-u2).^2);
            dp  = dp + fval*(p - p*XI2aXJ2(k) + 2*XIXJ(k));
            du1 = du1 + fval*(zi - p*zj);
            du2 = du2 + fval*(zj - p*zi);
            do1 = do1 + fval*(XI2aXJ2(k) - 1 + XI2mXJ2(k)/sqrtpr);
